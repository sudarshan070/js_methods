...
console.log(doesWordExist(words2, "matter")); /// true
...
console.log(howManyTimes(words3, "subset"));
...
var totalPoluation = data.reduce((acc, cv) => {
  if (cv.country == "China") {
    return acc;
  } else {
    return acc + cv.pop;
...
var allFruits = fruitBasket.reduce(function(acc, cv) {
  if (cv in acc) {
    acc[cv]++;
  } else {
    acc[cv] = 1;
